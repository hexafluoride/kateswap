...
(module token1 TOK1
  ...
  (defun mint (account:string amount:decimal)
    (enforce (> amount 0.0)
      "must mint more than 0 tokens")

    (with-capability (MINT)
      ;;(select ledger ['account,'balance] (where 'account (= "Fatima")))
      ;;(select ledger ['account,'balance] (where 'account (= account)))
      ;;(if (= (length (select ledger ['account,'balance] (where 'account (= account)))) 0 ) (create-account account) 0)
